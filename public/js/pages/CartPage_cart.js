...
async function refreshCartDisplay() {
    try {
        ...
        if (countEl)
            countEl.textContent = `${itemsCount} шт.`;
        // Обновляем цену
        const priceEl = document.getElementById('total-price');
        if (priceEl)
            priceEl.innerHTML = `<i class="fas fa-coins"></i> ${cartTotal.toLocaleString()}`;
        const totalPriceEl = document.querySelector('.total-price-value');
        if (totalPriceEl)
            totalPriceEl.innerHTML = `<i class="fas fa-coins"></i> ${cartTotal.toLocaleString()}`;
